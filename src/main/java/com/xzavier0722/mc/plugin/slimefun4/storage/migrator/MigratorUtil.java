...
class MigratorUtil {
    protected static boolean createDirBackup(File dir) {
        try {
            File oldDataDir = new File("data-storage/Slimefun/old_data/");
            oldDataDir.mkdirs();
            String backupPath = Path.of("data-storage/Slimefun/old_data/" + dir.getName() + ".zip");
            ...
            Object zipPath = Files.createFile(backupPath);
            try (ZipOutputStream zs = new ZipOutputStream(Files.newOutputStream(zipPath))) {
                Object src = dir.toPath();
                try (Stream fs = Files.walk(src).filter(path -> !Files.isDirectory(path))) {
                    fs.forEach(path -> {
    ...
    protected static boolean checkMigrateMark() {
        File backupData = new File("data-storage/Slimefun/old_data/");
